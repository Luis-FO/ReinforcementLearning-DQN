...
(BASE_DIR / "VideosLunarLander_Continuous_TD3_Test").mkdir(parents=True, exist_ok=True)
...
actor.load(f"{BASE_DIR}/model/LunarLander_actor_td3_v2.pt")
critic_1 = Critic(state_dim, action_dim).to(device)
critic_1.load(f"{BASE_DIR}/model/LunarLander_critic_1_td3_v2.pt")
critic_2 = Critic(state_dim, action_dim).to(device)
critic_2.load(f"{BASE_DIR}/model/LunarLander_critic_2_td3_v2.pt")
...
name_prefix = "LunarLander-TD3_test_v2"

# Setup environment with InfoOverlay and RecordVideo
env = gym.make(ENV_NAME, render_mode="rgb_array")
env = InfoOverlay(env, format_type = format_type)
env = RecordVideo(env, video_folder=f"{BASE_DIR}/VideosLunarLander_Continuous_TD3_v2_Test", name_prefix=name_prefix, episode_trigger=record_trigger)
